...
set JAVA_CMD=java -Dprogram.name=sftpd %SFTPD_OPTS% -Xmx%SFTPD_MEM_MB%m -Dsftp.id=%ID% -Dsftp.home=%SFTPD_HOME% -cp %SFTPD_HOME%/conf/%ID%/;%SFTPD_HOME%/conf/;%SFTPD_HOME%/lib;%SFTPD_HOME%/lib/* -Djava.security.manager -Djava.security.policy=file:%SFTPD_POLICY% %PWD_CLASS%
